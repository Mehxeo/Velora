...
// Seed with alpha.81, then verify using a manually installed candidate. No native updater installation is invoked.
  ...
  try {
    ...
    if(mode==='seed'){
      ...
      const fixture=await evaluate(`(async()=>{
        ...
        const message=await window.velora.chat.append({chatId:chat.id,role:'user',content:'Preserve this exact alpha.81 message: punctuation, **Markdown**, and Ω.'});
      ...
      assert.equal(version,'3.0.0-alpha.82');
