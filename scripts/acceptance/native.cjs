...
const windowsCandidate=`Velora-Setup-3.0.0-alpha.82.1${installerKind==='universal'?'':'-'+process.arch}.exe`;
assert.equal(tag,'v3.0.0-alpha.82.1','This acceptance suite is pinned to the alpha.82 candidate');
...
(async()=>{
 ...
 assert.equal(manifest.desktopSourceCommit,'5abf4ebddf18d6f9631ee7fbd7f4a167c0b8fbc1');
 // Fetch all expiring URLs before time-consuming install/UI tests.
 const artifactNames=process.platform==='darwin'?[`Velora-3.0.0-alpha.82.1${process.arch==='arm64'?'-arm64':''}-mac.zip`,`Velora-3.0.0-alpha.82.1${process.arch==='arm64'?'-arm64':''}.dmg`,'latest-mac.yml',`velora-cli-macos-${process.arch}.zip`]:[windowsCandidate,'velora-cli-windows-x64.zip'];
 ...
 if(process.platform==='darwin'){
  ...
  for(const name of [`Velora-3.0.0-alpha.82.1${suffix}-mac.zip`,`Velora-3.0.0-alpha.82.1${suffix}.dmg`,'latest-mac.yml']){download(tag,candidate,name);verify(name);}
  ...
  run('xcrun',['stapler','validate',path.join(candidate,`Velora-3.0.0-alpha.82.1${suffix}.dmg`)]);
  run('hdiutil',['attach',path.join(candidate,`Velora-3.0.0-alpha.82.1${suffix}.dmg`),'-readonly','-nobrowse','-mountpoint',mount]);
 ...
 const cliFile=path.join(cliDir,cliName+(process.platform==='win32'?'.exe':''));const version=run(cliFile,['--version']).trim();assert(version.includes('3.0.0-alpha.82.1'));
