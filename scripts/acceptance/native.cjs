...
const windowsCandidate=`Velora-Setup-3.0.0-alpha.82.5${installerKind==='universal'?'':'-'+process.arch}.exe`;
assert.equal(tag,'v3.0.0-alpha.82.5','This acceptance suite is pinned to the alpha.82 candidate');
...
(async()=>{
 ...
 assert.equal(manifest.desktopSourceCommit,'93a4d5e3985a1465c97aac08ce8bd3464275662c');
 // Fetch all expiring URLs before time-consuming install/UI tests.
 const artifactNames=process.platform==='darwin'?[`Velora-3.0.0-alpha.82.5${process.arch==='arm64'?'-arm64':''}-mac.zip`,`Velora-3.0.0-alpha.82.5${process.arch==='arm64'?'-arm64':''}.dmg`,'latest-mac.yml',`velora-cli-macos-${process.arch}.zip`]:[windowsCandidate,'velora-cli-windows-x64.zip'];
 ...
 if(process.platform==='darwin'){
  ...
  const old=`Velora-3.0.0-alpha.82.4${suffix}-mac.zip`;download('v3.0.0-alpha.82.4',previous,old);
  ...
  for(const name of [`Velora-3.0.0-alpha.82.5${suffix}-mac.zip`,`Velora-3.0.0-alpha.82.5${suffix}.dmg`,'latest-mac.yml']){download(tag,candidate,name);verify(name);}
  ...
  run('xcrun',['stapler','validate',path.join(candidate,`Velora-3.0.0-alpha.82.5${suffix}.dmg`)]);
  run('hdiutil',['attach',path.join(candidate,`Velora-3.0.0-alpha.82.5${suffix}.dmg`),'-readonly','-nobrowse','-mountpoint',mount]);
  ...
  const old=`Velora-Setup-3.0.0-alpha.82.4-x64.exe`;const installer=windowsCandidate;
  download('v3.0.0-alpha.82.4',previous,old);download(tag,candidate,installer);verify(installer);
 ...
 const cliFile=path.join(cliDir,cliName+(process.platform==='win32'?'.exe':''));const version=run(cliFile,['--version']).trim();assert(version.includes('3.0.0-alpha.82.5'));
