...
(async () => {
  ...
  try {
    ...
    assert.equal(download.to,'3.0.0-alpha.82.1');
    ...
    while(Date.now()<deadline){
      ...
      if(installedVersion==='3.0.0-alpha.82.1' && launched){relaunchPid=Number(launched[1]);break;}
      await new Promise(resolve=>setTimeout(resolve,500));
    }
    assert.equal(installedVersion,'3.0.0-alpha.82.1','Squirrel did not replace application');
    if(!relaunchPid){
